...
function lettersChangeNums(str) {
    ...
    let arr = strToArr.filter(str => str.length > 0);
    ...
    for (let el of arr) {
        ...
        let num = Number(numsAsStr);

        if (firstDigit == firstDigit.toUpperCase()) {
            let letterPosition = alphabetArray.indexOf(firstDigit);
            letterPosition += 1;
            result += num / letterPosition;
        } else if (firstDigit == firstDigit.toLowerCase()) {
            let digit = firstDigit.toUpperCase();
            let letterPositon = alphabetArray.indexOf(digit);
            letterPositon += 1;
            result += num * letterPositon;
        }

        if (lastDigit == lastDigit.toUpperCase()) {
            let letterPosition = alphabetArray.indexOf(lastDigit);
            letterPosition += 1;
            result -= letterPosition
        } else if (lastDigit == lastDigit.toLowerCase()) {
            let digit = lastDigit.toUpperCase();
            let letterPositon = alphabetArray.indexOf(digit);
            letterPositon += 1;
            result += letterPositon;
        }
        finalResult += result;
        result = 0;
    }
