...
function shootForWin(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        if (arr[i] != 'End') {
            let indx = Number(arr[i]);
            if (indx >= 0 && indx < targetSequence.length) {
                shotTargets++;
                let removedItem = targetSequence.splice(indx, 1, -1);

                for (let z = 0; z < targetSequence.length; z++) {
                    let num = removedItem[0];
                    if (targetSequence[z] > num && targetSequence[z] != -1) {
                        targetSequence[z] -= num;
                    } else if (targetSequence[z] <= num && targetSequence[z] != -1) {
                        targetSequence[z] += num;
                    }
                }
            }
